...
Config::Config(const char **argv, const int argc) {
	...
	parser.add("", 0, 1, 0, "Number of instances of the database.", "-ni"); // Number of instances
	...
	if (this -> nInstances < 4) {
		fprintf(stderr, "Error: The number of instances must be 4 or higher\n");
		exit(-1);
	}
	...
	if (this -> nGenerations == 0 && this -> nMigrations > 1) {
		this -> nMigrations = 1;
		fprintf(stderr, "Warning: If the number of generations is 0, the number of migrations must be 1. The number of migrations has been set to 1\n");
	}
