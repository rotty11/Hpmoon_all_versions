...
Config::Config(const int argc, const char **argv) {
	...
	CmdParser parser("High computing: Genetic algorithm in OpenCL and OpenMP, and distributed using OpenMPI.", "mpirun [MPI OPTIONS] ./bin/hpmoon [ARGS]", "Cluster HPMoon (C) 2018 v7.0");
	parser.addExample("./bin/hpmoon -h");
	parser.addExample("./bin/hpmoon -l");
	parser.addExample("mpirun --bind-to none --map-by node --host localhost,localhost ./bin/hpmoon -conf \"config.xml\" -ns 2 -trdb \"db/TRdata.txt\"");
	parser.addExample("mpirun --bind-to none --map-by node --host node0,localhost ./bin/hpmoon -conf \"config.xml\" -ss 480 -ngm 3 -nlm 2 -trdb \"db/TRdata.txt\" -trnorm");
	...
	check(this -> nLocalMigrations < 1, "%s\n", CFG_ERROR_MIGRATIONS_MIN);
	...
	check(this -> nGenerations == 0 && (this -> nGlobalMigrations > 1 || this -> nLocalMigrations > 1), "%s\n", CFG_ERROR_GENERATIONS_ONE);
