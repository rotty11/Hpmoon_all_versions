 ...
 * @param nFeatures The number of features (columns) of the database
 */
void transposeDataBase(float *dataBase, const int nInstances, const int nFeatures) {


	/********** Transpose database ***********/

	float traspose[nInstances * nFeatures];
	for(int f = 0, size = 0; f < nFeatures; ++f) {
		for (int i = 0; i < nInstances; ++i, ++size) {
			traspose[size] = dataBase[(nFeatures * i) + f];
		}
	}

	for(int s = 0; s < nFeatures * nInstances; ++s) {
		dataBase[s] = traspose[s];
	}
}