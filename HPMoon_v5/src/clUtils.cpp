	...
	for (int dev = 0; dev < conf -> nDevices; ++dev) {
		...
		for (int allDev = 0; allDev < allDevices.size() && !found; ++allDev) {
			...
			if (conf -> devices[dev] == dbuff) {
				...
				fstream kernels(conf -> kernelsFileName.c_str(), fstream::in);
				if (!kernels.is_open()) {
				...
				devices[dev].objSubpopulations = clCreateBuffer(devices[dev].context, CL_MEM_READ_WRITE, conf -> totalIndividuals * sizeof(Individual), 0, &status);
				if (status != CL_SUCCESS) {
					fprintf(stderr, "Error: Could not create the OpenCL object containing the subpopulations\n");
					exit(-1);
				}

				// Sets kernel arguments
				if (clSetKernelArg(devices[dev].kernel, 0, sizeof(cl_mem), (void *)&(devices[dev].objSubpopulations)) != CL_SUCCESS) {
