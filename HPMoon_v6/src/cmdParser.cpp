 ...
 * @date 02/01/2018
 * @brief File with the necessary implementation to parser the command line arguments
 */
...
bool CmdParser::addArg(const char *const arg, const bool requireValue, const char *const description) {
	...
	if (success) {
		std::pair<std::map<std::string, Argument>::iterator, bool> ret;
		ret = this -> arguments.insert(std::pair<std::string, Argument>(arg, Argument(description, requireValue)));
		success = ret.second;
	}
	return success;
}
...
bool CmdParser::parse(const char *const *const argv, const int argc) {
	...
	int parsed = 0;
	for(std::map<std::string, Argument>::iterator it = this -> arguments.begin(); it != this -> arguments.end() && success; ++it) {
...
void CmdParser::printHelp() {
	...
	if (!this -> arguments.empty()) {
		...
		int maxLength = 0;
		for(std::map<std::string, Argument>::iterator it = this -> arguments.begin(); it != this -> arguments.end(); ++it) {
			maxLength = (it -> first.length() > maxLength) ? it -> first.length() : maxLength;
		}
		...
		if (!this -> examples.empty()) {
			out += "\nEXAMPLES:\n\n";
			for (std::vector<std::string>::iterator it = this -> examples.begin(); it != this -> examples.end(); ++it) {
				out += *it + '\n';
			}
		}
	}
...
template<> int CmdParser::getValue(const char *const arg) {
	...
	int aux = (this -> find(arg)) ? atoi(this -> arguments.find(arg) -> second.getValue().c_str()) : aux;
	return aux;
}
...
template<> float CmdParser::getValue(const char *const arg) {
	...
	float aux = (this -> find(arg)) ? atof(this -> arguments.find(arg) -> second.getValue().c_str()) : aux;
	return aux;
}
...
template<> double CmdParser::getValue(const char *const arg) {
	...
	double aux = (this -> find(arg)) ? atof(this -> arguments.find(arg) -> second.getValue().c_str()) : aux;
	return aux;
}